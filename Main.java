...
public class Main {
    ...
    private static void addingNewItem() {
        boolean flag = true;
        while (flag) {
    ...
    private static boolean checkIfSensorNameExit(String sensorName) {
        for (Sensors sensor : sensorsList) {
            if (sensor.getName().equals(sensorName))
                return true;
        }
    ...
    private static boolean checkIfSensorIdExit(Integer sensorId) {
        for (Sensors sensor : sensorsList) {
